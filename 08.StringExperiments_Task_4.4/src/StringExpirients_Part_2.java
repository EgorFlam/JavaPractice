public class StringExpirients_Part_2 {
    public static void main(String[] args) {
        ...
        String word = text.replaceAll("[\\pP]", ""); // удаляем всю пунктуацию ([!\"#$%&'()*+,-./:;<=>?@\\[\\]^_`{|}~]) внутри текста с помощью символьного класса [\pP]

        // ДЛЯ СПРАВКИ: символьный класс [\w] спользуется для удаления буквенного текста, не затрагивая пунктуацию

        String[] wordWithoutPunct = word.split("\\s+"); //делим текст на слова по пробелам и результат выводим в виде массива

        for (int i = 0; i < wordWithoutPunct.length; i++) { // распечатаем массив строк с помощью цикла for

            System.out.println(wordWithoutPunct[i]); // выводим на печать каждое слово.
        }
    }
}
